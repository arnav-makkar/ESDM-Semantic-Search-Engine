...
def load_cache(pdf_folder):
    """Loads cached data if valid, otherwise returns None."""
    ...
    current_hash = get_pdf_folder_hash(pdf_folder)
    if cache_info.get("pdf_folder_hash") != current_hash:
        return None
    
    try:
        document_chunks = json.load(open(CHUNKS_FILE, "r"))
        metadata = json.load(open(METADATA_FILE, "r"))
        file_to_company = json.load(open(FILE_TO_COMPANY_FILE, "r"))
...
def save_cache(pdf_folder, document_chunks, metadata, file_to_company, embeddings):
    """Saves data to cache."""
    cache_info = {"pdf_folder_hash": get_pdf_folder_hash(pdf_folder)}
    ...
    with open(FILE_TO_COMPANY_FILE, "w") as f:
...
def extract_company_name(text):
    ...
    # First, check for "ORG" entities that contain "solar" in their text
    for ent in doc.ents:
        if ent.label_ == "ORG" and "solar" in ent.text.lower():
            return ent.text
    
    # Then, check for "ORG" entities followed by "Solar"
    for i, token in enumerate(doc):
        if token.text.lower() == "solar" and i > 0:
            prev_token = doc[i-1]
            if prev_token.ent_type_ == "ORG":
                start = prev_token.i
                while start > 0 and doc[start-1].ent_type_ == "ORG":
                    start -= 1
                company_name = doc[start:i].text + " Solar"
                return company_name
